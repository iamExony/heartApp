...
#FIREBASE CONFIG
cred = credentials.Certificate("streamlitauth-8403e-222031e90566.json")
#firebase_admin.initialize_app(cred) 


#SIDEBAR
def app():
    header = "Welcome to HEART APP" 
    content = "login to check your result, or create an account if you're a new user"

# Usernm = []
    st.sidebar.title('HEART APP :heart:')
    ...
    def sign_up_with_email_and_password(email, password, username=None, return_secure_token=True):
        try:
            ...
            try:
                ...
                st.sidebar.warning(r.json())
        except Exception as e:
            st.sidebar.warning(f'Signup failed: {e}')
    ...
    def sign_in_with_email_and_password(email=None, password=None, return_secure_token=True):
        ...
        try:
            ...
            st.sidebar.warning(f'Signin failed: {e}')
    ...
    def forget():
        ...
        if st.sidebar.button('Send Reset Link'):
            ...
            if success:
                st.sidebar.success("Password reset email sent successfully.")
            else:
                st.sidebar.warning(f"Password reset failed: {message}") 
    ...
    if  not st.session_state["signedout"]: # only show if the state is False, hence the button has never been clicked
        ...
        if choice == 'Sign up':
            username = st.sidebar.text_input("Enter  your full name")
